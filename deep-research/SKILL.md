...
1. **学术诚信闸门**：原本由 hooks 强制阻断的学术诚信闸门（各检查点与写权限围栏）现在以提示词级软约束执行，模型必须自觉在相应阶段执行这些检查清单，不得因缺乏强制机制而跳过。
2. **触发方式**：原斜杠命令（`ars-*`）改为自然语言触发，详见同目录 `COMMANDS.md`。
3. **子代理编排**：文中 "subagent / agent team" 指编排多个子任务；在支持子代理的编辑器中可并行派发执行，不支持时按顺序逐个执行即可。
...
研究问题成型后需要跑实验验证时，将工作移交给 autonomous-research 技能（E:\ARS-universal-skills\autonomous-research\SKILL.md）：其 Phase A 假设账本要求每条假设标注文献来源，可由本技能的文献扫描与 fact-check 模式供给。
...
Routing into Mode B requires explicit user signal — a natural-language mode request (formerly the `/ars-<mode>` slash command; see `COMMANDS.md`) or a `[direct-mode]` prefix. Ambiguous cross-phase input defaults to clarification per `shared/references/intent_clarification_protocol.md`.

**Enforcement (v3.9.2):** Phase Boundary blocks on Bucket A agents + advisory verifier (`scripts/check_pipeline_integrity.py`) + a deterministic write-scope guard in runtimes that support tool-call interception (#134 rescope, PR #294). Multi-phase envelope remains forward-scope (#134 Slices 3-5).
