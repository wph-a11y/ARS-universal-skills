...
**Enforcement (v3.9.4):** prompt-level fence + advisory verifier (`scripts/check_pipeline_integrity.py` v3.9.4 extension). Since the #134 rescope (PR #294), a deterministic write-scope guard enforces the WRITE clause in runtimes that support tool-call interception; where unavailable, this fence is the enforcement layer.
