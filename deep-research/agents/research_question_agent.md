...
**Enforcement (v3.9.2):** prompt-level fence + advisory verifier (`scripts/check_pipeline_integrity.py`). Since the #134 rescope (PR #294), a deterministic write-scope guard enforces the WRITE clause in runtimes that support tool-call interception; where unavailable, this fence is the enforcement layer.
