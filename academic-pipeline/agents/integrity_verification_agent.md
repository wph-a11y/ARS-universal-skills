...
it. Never pass a file path, arbitrary prompt, primary-model verdict, or unrelated paper
...
- Disagreements (primary-model `VERIFIED` vs cross-model `NOT_FOUND` / `MISMATCH`) → `[CROSS-MODEL-DISAGREEMENT]` → prioritized for human review
- `NOT_SEARCHED` / ungrounded results **never count as agreement** with a primary-model `VERIFIED`: count them separately and surface them for re-run or human review — an ungrounded cross-model verdict carries no evidence and must not be laundered into a confirmation
