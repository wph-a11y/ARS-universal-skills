...
- **学术诚信闸门（Stage 2.5 / Stage 4.5）**：原本由 hooks 强制阻断，现以提示词级软约束执行。模型必须自觉在相应阶段执行这些检查清单，并在报告中明确给出闸门结论（PASS / FAIL 及阻断项）。
- **原斜杠命令（`ars-*`）**：改为自然语言触发，触发词与用法详见同目录 `COMMANDS.md`。
- **"subagent / agent team"**：指编排多个子任务。在支持子代理的编辑器中可并行派发执行；不支持时按顺序执行即可，流程语义不变。
...
Stage 1（研究）与 Stage 2（写作）之间的实验空档由 autonomous-research 技能（E:\ARS-universal-skills\autonomous-research\SKILL.md）填补：其 Phase E 交接包接入 Stage 2.5/4.5 诚信闸门核验，实验主张与凭证逐条比对（ALIGNED / OVERSTATED / NOT_SUPPORTED_BY_PROVENANCE）。
...
- **Intent**: Invoke in a *fresh* session (a new conversation in your LLM editor). Resuming within the same session that emitted the boundary provides no token savings and may drop still-live in-session context.
