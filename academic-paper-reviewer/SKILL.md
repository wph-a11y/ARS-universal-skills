...
本技能已适配为通用 LLM 编辑器技能（TraeCode / Cursor / Copilot 等），不依赖 Claude Code 的 hooks、斜杠命令和子代理机制：

- 原本由 hooks 强制阻断的学术诚信闸门（如写权限保护、阶段边界检查）现在以提示词级软约束执行。模型必须自觉在相应阶段执行这些检查清单，不得因缺少机械强制而跳过。
- 原斜杠命令（`ars-*`）改为自然语言触发，详见同目录 `COMMANDS.md`。
- 文中 "subagent / agent team" 指编排多个子任务：在支持子代理的编辑器中可并行派发执行，不支持时按顺序执行即可。
...
> **Routing discipline (v3.9.2):** see the ARS "Routing Discipline (v3.9.2)" rules + `shared/references/intent_clarification_protocol.md` for cross-skill routing rules. This skill assumes routing has already settled — ambiguous cross-phase materials should have been clarified upstream.

## 自主科研衔接（autonomous-research）

评审含实验的论文时，可要求作者提供 autonomous-research 技能（E:\ARS-universal-skills\autonomous-research\SKILL.md）的假设账本与实验凭证作为评审证据；无法提供凭证的实验主张按 HIGH-WARN 标注。
...
**Mode B — phase-by-phase (cross-session resume):** User invokes one reviewer agent per phase across sessions, or runs the full reviewer panel standalone via natural-language triggers (formerly the `/ars-review` slash command; see `COMMANDS.md`).
...
Routing into Mode B requires an explicit user signal — a natural-language mode request (formerly the `/ars-<mode>` slash commands; see `COMMANDS.md`) or a `[direct-mode]` prefix. Ambiguous cross-phase input defaults to clarification per the Routing Discipline rules + `shared/references/intent_clarification_protocol.md`.

**Enforcement (v3.9.2):** Phase Boundary blocks on Bucket A agents + advisory verifier (`scripts/check_pipeline_integrity.py`) + a deterministic write-scope guard where the editor runtime supports tool-call interception (prompt-level soft constraint elsewhere) (#134 rescope, PR #294). Multi-phase envelope remains forward-scope (#134 Slices 3-5).
